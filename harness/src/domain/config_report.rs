...
mod tests {
    ...
    fn a_code_default_renders_as_a_parenthesized_sentinel_not_a_path() {
        ...
        assert_eq!(
            ConfigSource::File("/p/.bdd.toml".into()).display(),
            "/p/.bdd.toml"
        );
    }
    ...
    fn text_render_is_tab_separated_key_value_source() {
        ...
        let text = build_report(
            ConfigFileStatus::Present {
                path: "/p/.bdd.toml".into(),
            },
            present,
        )
        .to_string();
        assert!(text.starts_with("file\t/p/.bdd.toml\n"));
        assert!(text.contains("llm.model\tqwen\t/p/.bdd.toml\n"));
