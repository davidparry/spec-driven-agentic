...
[![bdd harness](https://img.shields.io/github/v/release/davidparry/tdd-bdd-agentic?label=bdd%20harness)](https://github.com/davidparry/tdd-bdd-agentic/releases/latest)
...
with narrower per-command tool profiles (`bdd tools profiles`), not a second
implementation.
...
prompting, skills, and review. The `bdd` commands are a *spec-specific runner*
...
60-minute workshop, or [`slides/index.html?30`](slides/index.html) for the
30-minute demo-driven cut. One file, two tracks — see
[`slides/index.html`](slides/index.html) `data-track`.
...
| `.cursor/mcp.json` / [`.mcp.json`](.mcp.json) / [`.pi/mcp.json`](.pi/mcp.json) / [`config/mcp.json`](config/mcp.json) | Registers `bdd mcp serve` with Cursor (`.cursor/mcp.json`), Claude Code (`.mcp.json` + `.claude/settings.json`), [pi](https://pi.dev) (`.pi/mcp.json`, read by `pi-mcp-extension`; no `--root`, so launch pi from the repo root), and other MCP hosts. |
...
- **harness** — `bdd` tests, clippy, fmt, coverage (≥ 97% library lines, `main.rs` ignored), and the release binary.
- **smoke-test-live** — needs `harness`; drives `smoke-test.jar` against that binary (`-Dbdd.binary=…`) so the child's process is proven to be real `bdd mcp serve`.
...
1. **Harness** — format check, clippy with warnings as errors, the full
   ...
   (`harness/target/release/bdd`). The same gates CI enforces.
...
- **`bdd` on PATH** (GitHub release, `cargo install --path harness`, or `harness/target/release/bdd`)
- Java 21+
- Maven 3.9+
...
`harness/target/debug/`. It does **not** copy `bdd` onto PATH. Cursor's
...
debug binary without installing: `harness/target/debug/bdd mcp serve --root .`.
...
`bdd mcp serve` exposes **25 tools**. Cursor sees all of them, and so does
`pi --no-builtin-tools`. Harness commands that call a model attach a scoped
profile (`bdd tools profiles`): 3–7 tools for a generating command, 12 for the
read-only `bdd ask`.

There is no `spec_draft` or `implement` MCP tool: a **new** requirement is
still drafted by the human (`bdd spec draft`), and Cursor writes production
...
IDE does: launch `bdd mcp serve`, `tools/list` (25 tools),
...
test, and the REQ-007 you drafted against `bdd spec validate` and
`bdd spec refine` rather than anyone else's wording.
