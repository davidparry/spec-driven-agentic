...
[`spec deliver`](commands/deliver.md) runs the same rhythm from a plan —
one id, a plain-words description, or the whole pending backlog — without
stopping to ask, and reports at the end which requirements landed, so a
single command can stand in for the whole block above:

```bash
spec deliver REQ-002
```
