...
Install `spec` first — [published installer](../README.md#install-spec), **0.5.5 or newer**.
...
spec --version                     # must succeed, and report 0.5.5 or newer
...
Run the exercise a step at a time — watching the calls arrive one by one
is the point, and **the room is the orchestrator**. No single command
stands in for the sequence here, because a command that collapses the
steps also collapses the checks, and the checks are what the hour is
...
`scripts/verify-workshop-run.sh check`, seven checks by name.
