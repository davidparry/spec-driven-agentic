...
/**
 * The workshop's smoke test of {@code spec mcp serve}: launches the server
 ...
 *
 * <p>Run it from the repo root after {@code spec} is installed and on
 * {@code PATH} ({@code spec --version} succeeds in the same shell):
...
public final class TddAgent {
    ...
    public static void main(String[] args) {
        ...
        Path spec = SpecBinary.onPath();
        if (spec == null) {
            System.out.println("spec was not found on PATH.");
            System.out.println("Install it: cargo install --path harness");
            System.out.println("Or add the directory that contains spec to PATH.");
            System.exit(1);
        }

        Narrator narrator = new Narrator(System.out::println);
        narrator.banner("STEP 0 — Launch the server");
        narrator.say("The client starts spec mcp serve as a child process and talks JSON-RPC 2.0 over stdin/stdout.");
        narrator.say("Command: " + spec + " mcp serve --root " + root);

        try (SdkToolClient client = new SdkToolClient(root, spec)) {
            if (isSweep(args)) {
